...
// --- CẤU HÌNH API ---
const API_BASE_URL = "http://localhost:8081"; // Quan trọng: URL tuyệt đối tới Backend
...
export default function DashboardOverview() {
    ...
    useEffect(() => {
        const fetchDashboardData = async () => {
            setLoading(true);
            try {
                // 1. Lấy Token đúng key như bên UserManagement
                ...
                // 2. Gọi API với đường dẫn tuyệt đối để tránh lỗi HTML
                ...
                // --- HÀM HELPER ĐỂ ĐỌC DỮ LIỆU AN TOÀN (GIỐNG USER MANAGEMENT) ---
                const extractDataList = (res) => {
                    if (res.status !== 'fulfilled') return [];
                    const d = res.value.data;
                    // Kiểm tra mọi trường hợp JSON Backend có thể trả về
                    if (Array.isArray(d)) return d;
                    if (d.Items && Array.isArray(d.Items)) return d.Items; // .NET thường trả về Items
                ...
                const extractCount = (res) => {
                    ...
                    // Ưu tiên lấy biến Total từ phân trang
                    if (typeof d.Total === 'number') return d.Total;
                    if (typeof d.total === 'number') return d.total;
                    if (typeof d.totalCount === 'number') return d.totalCount;
                    if (typeof d.TotalCount === 'number') return d.TotalCount;

                    // Nếu không có biến Total, đếm thủ công mảng
                ...
                setStats({
                    ...
                    papers: extractCount(papersRes),
                    courses: extractCount(coursesRes)
                });

                // Lấy danh sách user thật cho Modal
                setUserList(extractDataList(usersRes));

                // Map tên bài thi thật + Fake số liệu attempts (vì chưa có API Report)
                const paperList = extractDataList(papersRes);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
          <div className="flex justify-between items-center mb-4">
            ...
            {/* Email List Modal (REAL DATA from /admin/users) */}
            {showEmailModal && (
                ...
                <div className="bg-white rounded-xl shadow-lg p-8 w-full max-w-lg relative">
                    ...
                    <h2 className="text-xl font-bold mb-6 text-slate-800">Real Users from API</h2>
